...
#    render :template => "visits/visit_search"
      ...
      if v_include_radiology_comments == "1"
         ...
         case  request_format
           when "[text/html]","text/html" then
             ...
             @html_request ="N"
             @column_headers = ['Date','Protocol','Enumber','RMR','Scan','Path',  'Completed Fast','Fast hrs','Fast min','Last Nicotine hrs','Last Nicotine min','Last Caffeine hrs','Last Caffeine min','MRI start Hour','MRI start Minute','MRI end Hour','MRI end Minute','Mri status','Radiology Outcome','Notes','BP Systol','BP Diastol','Pulse','Blood Glucose','Age at Appt','Appt Note'] # need to look up values
             # Protocol,Enumber,RMR,Appt_Date get prepended to the fields, appointment_note appended
             @column_number =   @column_headers.size
             @fields =["visits.scan_number","visits.path","CASE visits.completedmrifast WHEN 1 THEN 'Yes' ELSE 'No' end",
               "visits.mrifasttotaltime","visits.mrifasttotaltime_min","visits.mrilast_nicotine_totaltime","visits.mrilast_nicotine_totaltime_min","visits.mrilast_caffeine_totaltime","visits.mrilast_caffeine_totaltime_min","visits.mristarttime_hour","visits.mristarttime_minute","visits.mriendtime_hour","visits.mriendtime_minute","vgroups.transfer_mri","radiology_outcome","visits.notes","vitals.bp_systol","vitals.bp_diastol","vitals.pulse","vitals.bloodglucose","appointments.age_at_appointment","visits.id"] # vgroups.id vgroup_id always first, include table name
  ...
  def set_current_tab
   ...
   def visit_params
          params.require(:visit).permit(:buttonbox,:goggles,:mriendtime,:mristarttime,:mritech,:appointment_id,:temp_fkMRIscanid,:vgroup_id,:completedmrifast,:archivedvd,:mriendtime_minute,:mriendtime_hour,:mristarttime_minute,:mristarttime_hour,:mri_coil_name,:use_as_default_mri_flag,:mrifasttotaltime_min,:mrifasttotaltime,:mrilast_nicotine_totaltime_min,:mrilast_nicotine_totaltime,:mrilast_caffeine_totaltime_min,:mrilast_caffeine_totaltime,:compiled_at,:dicom_study_uid,:created_by_id,:transfer_mri_moved_to_vgroups,:notes,:radiology_outcome,:rmr,:initials,:scan_number,:date,:id,:transfer_pet_moved_to_vgroups,:conference,:scanner_source,:consent_form_type,:research_diagnosis,:radiology_note,:path,:user_id,:dicom_dvd,:compile_folder_moved_to_vgroups, :enrollments_attributes, scan_procedure_ids: []) # [:_destroy,:enumber,:id])
   end   
