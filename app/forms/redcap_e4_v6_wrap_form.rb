class RedcapE4V6WrapForm
	...
	def self.from_csv (row)
		...
		out = RedcapE4V6WrapForm.new

		out.lumbarpuncture_fields = {}
		out.vitals_fields = {}
		out.appointment_fields = {}

		out.enumber = "wrap#{row["ptid"].rjust(4,'0')}" 
		out.appointment_fields["appointment_date"] = Date.strptime(row["lpdt_ev_v6"], "%Y-%m-%d")
		# "formver_e4", 
		...
		out.lumbarpuncture_fields["lpothersideeffects_severity"] = case row["othreff_sev_e4_v6"].to_i
		when 1
			'mild (0-3/10)'
		when 2
			'moderate (4-6/10)'
		...
		out.lumbarpuncture_fields["lp_exam_md_id"] = case row["lpmd_e4_v6"].to_i
		when 1
