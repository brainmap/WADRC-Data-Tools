...
#      protocols: ["carlsson.brave.visit1"],
  ...
  def track_file(params)
    ...
    if create_return[:status] == "track_success"
      ...
      log = "#{Time.now.strftime("%Y-%m-%d_%H:%M:%S")}\nCreateTrCg #{params[:category]} START:\n#{create_return[:log]}TRACK: #{params[:category]} file needs QC.\n"
      return log
    elsif create_return[:status] == "qc_fail"
      log = "#{Time.now.strftime("%Y-%m-%d_%H:%M:%S")}\nCreateTrCg #{params[:category]} START:\n#{create_return[:log]}TRACK: #{params[:category]} file did not pass QC. SKIPPING TRACKING!\n"
      return log
    end
  end
  ...
  def setup(params)
    ...
    if ActiveRecord::Base.connection.table_exists?("#{params[:destination_table]}_new")
      sql = "TRUNCATE TABLE #{params[:destination_table]}_new;"
      ...
      subject_dirs.each do |subject|
       ...
       # cbf_names = Dir.glob("#{subject}*CBF*",:base=>subject_dir)
        ...
        cbf_paths.each do |cbf_nii|
          ...
          check_globs = {
            :wcg_tcCBF => "#{cbf_dir_path}/wcg_mri*_CBF.nii",
            :cg_tcCBF => "#{cbf_dir_path}/cg_mri*_CBF.nii",
            :tcCBF_nii => "#{cbf_dir_path}/*_CBF.nii"
          }
          ...
          html_glob ="#{cbf_dir_path}/*_output.html"
          ...
          cg_glob ="#{cbf_dir_path}/cg_mri*_CBF.nii"
  ...
  def rotate_tables(params)    
    ...
    ActiveRecord::Base.connection.execute(sql_new)
...
#create = "CREATE TABLE `cg_asl` (`id` INT NOT NULL AUTO_INCREMENT,`scan_procedure_id` INT DEFAULT NULL,`enrollment_id` INT DEFAULT NULL,`participant_id` INT DEFAULT NULL,`trfile_id` INT DEFAULT NULL,`processed_image_id` INT DEFAULT NULL,`neuromorph_cbf_metrics_id` INT DEFAULT NULL,`neuromorph_vgm_metrics_id` INT DEFAULT NULL, `research_cbf` BOOLEAN DEFAULT NULL, `gm_cbf_median` FLOAT DEFAULT NULL,`gm_cbf_mad` FLOAT DEFAULT NULL,`gm_cbf_mean` FLOAT DEFAULT NULL,`gm_cbf_std` FLOAT DEFAULT NULL,`wm_cbf_median` FLOAT DEFAULT NULL,`wm_cbf_mad` FLOAT DEFAULT NULL,`wm_cbf_mean` FLOAT DEFAULT NULL,`wm_cbf_std` FLOAT DEFAULT NULL,PRIMARY KEY (`id`));"
