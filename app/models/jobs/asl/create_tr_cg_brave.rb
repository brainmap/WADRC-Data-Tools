module CreateTrCgBrave
  ...
  def create_tr_cg_brave(tr_params)
    ...
    if tr_candidates.length == 1
      ...
      pre_check = ActiveRecord::Base.connection.execute(pre_check_sql)
      if pre_check.count < 1 #Add to cg_asl
        ...
        neuro_table[1].drop(1).each { |name| column_names << ",`#{name.downcase.tr(" ","_")}`" }
        #CBF
        cbf_values = neuro_table[3].drop(1).join(', ')
        cbf_values.gsub!("NaN", "NULL")
        #VGM
        vgm_values = neuro_table[2].drop(1).join(', ')
        ...
        #tr_id = Trfile.where("trtype_id in (?)",tr_params[:tracker_id]).where("subjectid in (?)",tr_params[:subject]).first.id #Already had trfile_id from above
        ...
        cg_check = "SELECT * FROM #{tr_params[:new_table]} WHERE id = #{asl_id};"
        cg_check_out = ActiveRecord::Base.connection.execute(cg_check)
        if cg_check_out.count < 1
