class Jobs::ImportRocheAnalytes < Jobs::BaseJob
	...
	def import(params)
		...
		# I was trying to fix this in code before, but looks like this is needed here as well.
		sql = "update #{params[:roche_table]}_new left join enrollments on enrollments.enumber = #{params[:roche_table]}_new.enumber set #{params[:roche_table]}_new.participant_id = enrollments.participant_id where #{params[:roche_table]}_new.participant_id is null";
		@connection.execute(sql)
