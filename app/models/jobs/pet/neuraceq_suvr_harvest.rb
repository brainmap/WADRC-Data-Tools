class Jobs::Pet::NeuraceqSuvrHarvest < Jobs::Pet::PetHarvestBase
  ...
  attr_accessor :harvester_logger

  def self.default_params
    # - set up params
    params = { schedule_name: 'pet_neuraceq_suvr_harvest',
                ...
                pipeline_id: 200,
                run_no_job_case: true,
                logger_dir: '/mounts/data/preprocessed/logs/pet_neuraceq',
                trtype_id: 39,
                sp_whitelist: [155],
  ...
  def run(params)
    ...
    begin
      @harvester_logger = ActiveSupport::Logger.new("#{params[:logger_dir]}/#{Time.now.strftime("%Y-%m-%d_%H:%M:%S")}_harvester.log")
...
#      rotate_tables(params)
      ...
      #      @harvester_logger.close()
      
    rescue StandardError => error

      @harvester_logger.error{"Error (#{error.class}): #{error.message}"}
      #      @harvester_logger.close()
...
#                                       "suvr_altref_cblm_gm_inf","volume_cc_altref_cblm_gm_inf","suvr_altref_centsem_wm","volume_cc_altref_centsem_wm"] #added columns
    #Adjusted so that a name change in the output csv file would still allow columns to be caught
    #added two more columns and then filter on sql insert for columns that don't return nil
    @roi_column_names['_atlas_na'] = ["suvr_altref_cblm_gm","suvr_altref_cblm_whole","suvr_altref_cblm_wm","suvr_altref_centsm_wm","volume_cc_altref_cblm_gm","volume_cc_altref_cblm_whole","volume_cc_altref_cblm_wm","volume_cc_altref_centsm_wm"]
...
#      if ActiveRecord::Base.connection.table_exists?()  
        ...
        results = @connection.execute(sql)
#      else
#        sql = "CREATE TABLE `#{table_name}_new` "
#        results = @connection.execute(sql)
#      end
    end
  ...
  def harvest(params)
    ...
    @harvester_logger.info{"Scan Procedures Harvested: #{scan_procedures.to_a}"}

      @scan_procedures.each do |sp|
      @harvester_logger.info{"Start: #{sp.codename}"}
#        self.log << "start "+sp.codename
        ...
        if File.directory?(v_preprocessed_full_path)
          ...
          enrollments = Enrollment.joins("LEFT JOIN enrollment_vgroup_memberships ON enrollment_vgroup_memberships.enrollment_id = enrollments.id")
                              ...
                              .uniq
          @harvester_logger.info{"Enrollments: #{enrollments.to_a}"}
          enrollments.each do |enrollment|
          @harvester_logger.info{"Harvesting: #{enrollment.enumber}"}
#            self.log << "starting #{enrollment.enumber}"

          #log actions for each participant
          pt_log_path = "#{params[:logger_dir]}/#{sp.codename.gsub('.','_')}_#{enrollment.enumber}_#{Time.now.strftime("%Y-%m-%d_%H:%M:%S")}_neuraceq_harvester.log"
          pt_log = ActiveSupport::Logger.new(pt_log_path)
          pt_log.info{"SUBJECT: #{enrollment.enumber}; PROTOCOL: #{sp.codename}"}


          if !params[:run_no_job_case]
            #Check job case for error or exclusion
            job_case = Jobs::JobCase.where("pipeline_id" == pipeline.id).where("enrollment_id" == enrollment.id).where("enumber" == enrollment.enumber).where("scan_procedure_id" == sp.id)
            if job_case.count == 1
              job_case = job_case.first
              if job_case.status == 'error' || job_case.status == 'exclusion'
                @harvester_logger.error{"#{sp.codename}/#{enrollment.enumber} Has Job Case status of #{job_case.status}! Skipping!"}
                pt_log.error{"#{sp.codename}/#{enrollment.enumber} Has Job Case status of #{job_case.status}! Skipping!"}
                next
              end
            elsif job_case.blank? 
              @harvester_logger.error{"#{sp.codename}/#{enrollment.enumber} Missing Job Case! Skipping!"}
              pt_log.error{"#{sp.codename}/#{enrollment.enumber} Missing Job Case! Skipping!"}
              next
            elsif job_case.count > 1
              @harvester_logger.error{"#{sp.codename}/#{enrollment.enumber} Has multiple Job Cases! Need to resolve! Skipping!"}
              pt_log.error{"#{sp.codename}/#{enrollment.enumber} Has multiple Job Cases! Need to resolve! Skipping!"}
              next
            elsif job_case.status == 'complete'
              @harvester_logger.info{"#{sp.codename}/#{enrollment.enumber} already completed."}
              pt_log.info{"#{sp.codename}/#{enrollment.enumber} already completed."}
              next
            end
          end
...
#                self.log << "IN RESCUE ERROR: #{msg}"
              @harvester_logger.error{"#{sp.codename}/#{enrollment.enumber}: #{msg}"}
              pt_log.error{msg}
            end
...
#                self.log << "tacs.csv is #{tacs_file_name}"
                ...
                pt_log.info{"product file is #{product_file_name}"}
#                self.log << "product file is #{product_file_name}"
                ...
                if !roi_file_name.blank? and !log_file_name.blank? and !product_file_name.blank?
                  ...
                  if log_file_data["tracer"] != @pettracer.name.downcase
                    @harvester_logger.error{"EXCLUSION: #{sp.codename}/#{enrollment.enumber}: {\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log file's tracer (#{log_file_data["tracer"]}) is != this job's tracer (#{@pettracer.name.downcase})\"}"}
                    pt_log.error{"{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log file's tracer (#{log_file_data["tracer"]}) is != this job's tracer (#{@pettracer.name.downcase})\"}. EXCLUDED"}
#                    self.exclusions << "{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log file's tracer (#{log_file_data["tracer"]}) is != this job's tracer (#{@pettracer.name.downcase})\"}"
                  ...
                  if log_file_data["study ID"] != enrollment.enumber
                    @harvester_logger.error{"EXCLUSION: #{sp.codename}/#{enrollment.enumber}: {\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's study ID (#{log_file_data["study ID"]}) is != this enumber (#{enrollment.enumber})\"}"}
                    pt_log.error{"{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's study ID (#{log_file_data["study ID"]}) is != this enumber (#{enrollment.enumber})\"}. EXCLUDED"}
#                    self.exclusions << "{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's study ID (#{log_file_data["study ID"]}) is != this enumber (#{enrollment.enumber})\"}"
                  ...
                  if !params[:code_version].include? log_file_data["PET code version"]
                    @harvester_logger.error{"EXCLUSION: #{sp.codename}/#{enrollment.enumber}: {\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's code version (#{log_file_data["PET code version"]}) is != this job's code version (#{params[:code_version]})\"}"}
                    pt_log.error{"{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's code version (#{log_file_data["PET code version"]}) is != this job's code version (#{params[:code_version]})\"}. EXCLUDED"}
#                    self.exclusions << "{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's code version (#{log_file_data["PET code version"]}) is != this job's code version (#{params[:code_version]})\"}"
                  ...
                  if log_file_data["method"] != params[:method]
                    @harvester_logger.error{"EXCLUSION: #{sp.codename}/#{enrollment.enumber}: {\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's method (#{log_file_data["method"]}) is != this job's method (#{params[:method]})\"}"}
                    pt_log.error{"{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's method (#{log_file_data["method"]}) is != this job's method (#{params[:method]})\"}. EXCLUDED"}
#                    self.exclusions << "{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's method (#{log_file_data["method"]}) is != this job's method (#{params[:method]})\"}"
                  ...
                  if log_file_data["protocol description"] != sp.codename
                    @harvester_logger.error{"EXCLUSION: #{sp.codename}/#{enrollment.enumber}: {\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's protocol description (#{log_file_data["protocol description"]}) is != this subject's scan procedure (#{sp.codename})\"}"}
                    pt_log.error{"{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"the log's protocol description (#{log_file_data["protocol description"]}) is != this subject's scan procedure (#{sp.codename})\"}. EXCLUDED"}
                    ...
                    if !v_original_t1_mri_file_array.nil? and v_original_t1_mri_file_array.count > 5
                      ...
                      if !mri_sps.nil? and !mri_enums.nil?  and mri_sps.count > 0 and mri_enums.count > 0
                        ...
                        if mri_appointments.count > 0 and mri_appointments.count < 2
                          ...
                          @harvester_logger.error{"EXCLUSION: #{sp.codename}/#{enrollment.enumber}: {\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"Too many MRI appointments? (count was #{mri_appointments.count})\"}"}
                          pt_log.error{"{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"Too many MRI appointments? (count was #{mri_appointments.count})\"}. EXCLUDED"}
#                          self.exclusions << "{\"class\":\"#{enrollment.class}\", \"id\":\"#{enrollment.id}\", \"message\":\"Too many MRI appointments? (count was #{mri_appointments.count})\"}"
                  ...
                  #Catch nil age_at_appointments
                  if v_age_at_appointment.empty?
                    v_age_at_appointment = "NULL"
                  end
                  ...
                  v_sql_check = "Select mri_processing_date,pet_processing_date,subjectid,enrollment_id,scan_procedure_id from cg_pet_#{@pettracer.name.downcase}_roi where enrollment_id = "+enrollment.id.to_s+" and scan_procedure_id = "+sp.id.to_s+" and secondary_key = '"+v_secondary_key.to_s+"'"
                  ...
                  if log_file_data["Bias Corrected T1 MRI file"]  > ""
                    ...
                    v_sql_check = "Select mri_processing_date,subjectid,enrollment_id,scan_procedure_id from cg_pet_#{@pettracer.name.downcase}_roi where enrollment_id = "+enrollment.id.to_s+" and scan_procedure_id = "+sp.id.to_s+" and secondary_key = '"+v_secondary_key.to_s+"'"
                    results_check = @connection.execute(v_sql_check) 
                  ...
                  if !roi_file_name.blank?
                    ...
                    if v_return_flag == "N"
                      @harvester_logger.error{"#{sp.codename}/#{enrollment.enumber}: ROI header didn't match: #{roi_file_name}=>#{v_return_comment}"}
                      pt_log.error{"ROI header didn't match: #{roi_file_name}=>#{v_return_comment}."}
#                      self.log << "ROI header didn't match: #{roi_file_name}=>#{v_return_comment}"
                      ...
                      roi_summary_data.each do |row|
                        ...
                        if atlas.nil?
                          @harvester_logger.error{"#{sp.codename}/#{enrollment.enumber}: UNEXPECTED ATLAS #{row["Atlas"]};"}
                          pt_log.error{"UNEXPECTED ATLAS #{row["Atlas"]};"}
#                          self.log << "UNEXPECTED ATLAS #{row["Atlas"]};"
                    ...
                    @harvester_logger.error{"#{sp.codename}/#{enrollment.enumber}: ERROR: missing ROI file, enumber: #{enrollment.id}, scan_procedure: #{sp.codename}"}
                    pt_log.error{"ERROR: missing ROI file, enumber: #{enrollment.id}, scan_procedure: #{sp.codename}"}
#                    self.log << "ERROR: missing ROI file, enumber: #{enrollment.id}, scan_procedure: #{sp.codename}" 
                ...
                preamble_fields = %w"file_name subjectid enrollment_id scan_procedure_id secondary_key pet_processing_date mri_processing_date pet_code_version original_t1_mri_file_name bias_corrected_t1_mri_file mni_space_t1_mri multispectral_file pet_log_file coregistration_reference_file processed_4d_pet_file processed_sum_pet_file processed_suvr_pet_file mni_space_suvr_pet_file pet_analysis_log_file ecat_file_name atlas age_at_appointment general_comment pet_date_mri_date_diff_days"
                  pt_log.info{"file name => #{roi_file_name.split("/").last.to_s}"}
                  pt_log.info{"subjectid => #{v_subjectid_v_num}"}
                  pt_log.info{"enrollment_id => #{enrollment.id.to_s}"}
                  pt_log.info{"scan_procedure_id => #{sp.id.to_s}"}
                  pt_log.info{"secondary_key => #{v_secondary_key.to_s}"}
                  pt_log.info{"pet_processing_date => #{pet_date.to_s}"}
                  pt_log.info{"mri_processing_date => #{mri_date.to_s}"}

                  pt_log.info{"pet_code_version => #{log_file_data["PET code version"]}"}
                  pt_log.info{"original_t1_mri_file_name => #{log_file_data["original t1 MRI file"].to_s}"}
                  pt_log.info{"bias_corrected_t1_mri_file => #{log_file_data["Bias Corrected T1 MRI file"]}"}
                  pt_log.info{"mni_space_t1_mri => #{log_file_data["MNI space T1 MRI"]}"}
                  pt_log.info{"multispectral_file => #{log_file_data["multispectral file"]}"}
                  pt_log.info{"pet_log_file => #{log_file_data["PET log file"]}"}
                  pt_log.info{"coregistration_reference_file => #{log_file_data["coregistration reference file"]}"}
                  pt_log.info{"processed_4d_pet_file => #{log_file_data["processed 4D PET file"]}"}
                  pt_log.info{"processed_sum_pet_file => #{log_file_data["processed SUM PET file"]}"}
                  pt_log.info{"processed_suvr_pet_file => #{log_file_data["processed SUVR PET file"]}"}
                  pt_log.info{"mni_space_suvr_pet_file => #{log_file_data["MNI space SUVR PET file"]}"}
                  pt_log.info{"pet_analysis_log_file => #{log_file_data["PET analysis log file"]}"}
                  pt_log.info{"ecat_file_name => #{ecat_file.to_s}"}
                  pt_log.info{"atlas => %{atlas_name}"}
                  pt_log.info{"age_at_appointment => #{v_age_at_appointment}"}
                  pt_log.info{"general_comment => #{v_qc_value}"}
                  pt_log.info{"pet_date_mri_date_diff_days => #{pet_date_mri_date_diff_days}"}
...
#                preamble_values = "\'#{roi_file_name.split("/").last.to_s}\', \'#{v_subjectid_v_num}\', #{enrollment.id}, #{sp.id}, \'#{v_secondary_key.to_s}\', \'#{pet_date.to_s}\', \'#{mri_date.to_s}\', \'#{log_file_data["PET code version"]}\', \'#{log_file_data["original t1 MRI file"].to_s}\', \'#{log_file_data["Bias Corrected T1 MRI file"]}\', \'#{log_file_data["MNI space T1 MRI"]}\', \'#{log_file_data["multispectral file"]}\', \'#{log_file_data["PET log file"]}\', \'#{log_file_data["coregistration reference file"]}\', \'#{log_file_data["processed 4D PET file"]}\', \'#{log_file_data["processed SUM PET file"]}\', \'#{log_file_data["processed SUVR PET file"]}\', \'#{log_file_data["MNI space SUVR PET file"]}\' , \'#{log_file_data["PET analysis log file"]}\', \'#{ecat_file.to_s}\', \'%{atlas_name}\', #{v_age_at_appointment.to_f}, \'#{v_qc_value}\', #{pet_date_mri_date_diff_days.to_i}"

                  column_name_changes = {"suvr_altref_cblm_gm":"suvr_altref_cblm_gm_inf","volume_cc_altref_cblm_gm":"volume_cc_altref_cblm_gm_inf","suvr_altref_centsm_wm":"suvr_altref_centsem_wm","volume_cc_altref_centsm_wm":"volume_cc_altref_centsem_wm"}
                  column_name_changes.each_pair do |orig, change|
                  if !v_roi_hash['_atlas_na'][change].nil?
                    v_roi_hash['_atlas_na']["#{orig}"] = v_roi_hash['_atlas_na'][change]
                    v_roi_hash['_atlas_na'].delete(change)
                  end
                end
...
#CREATE TABLE `cg_pet_neuraceq_roi_new` (`subjectid` varchar(50) DEFAULT NULL,`general_comment` varchar(2000) DEFAULT NULL,`enrollment_id` int(11) DEFAULT NULL,`scan_procedure_id` int(11) DEFAULT NULL,`file_name` varchar(500) DEFAULT NULL,`secondary_key` varchar(10) DEFAULT NULL,`atlas` varchar(500) DEFAULT NULL,`pet_processing_date` varchar(30) DEFAULT NULL,`pet_code_version` varchar(30) DEFAULT NULL,`ecat_file_name` varchar(500) DEFAULT NULL,`original_t1_mri_file_name` varchar(200) DEFAULT NULL,`suvr_precentral_l` varchar(30) DEFAULT NULL,`suvr_precentral_r` varchar(30) DEFAULT NULL,`suvr_frontal_sup_l` varchar(30) DEFAULT NULL,`suvr_frontal_sup_r` varchar(30) DEFAULT NULL,`suvr_frontal_sup_orb_l` varchar(30) DEFAULT NULL,`suvr_frontal_sup_orb_r` varchar(30) DEFAULT NULL,`suvr_frontal_mid_l` varchar(30) DEFAULT NULL,`suvr_frontal_mid_r` varchar(30) DEFAULT NULL,`suvr_frontal_mid_orb_l` varchar(30) DEFAULT NULL,`suvr_frontal_mid_orb_r` varchar(30) DEFAULT NULL,`suvr_frontal_inf_oper_l` varchar(30) DEFAULT NULL,`suvr_frontal_inf_oper_r` varchar(30) DEFAULT NULL,`suvr_frontal_inf_tri_l` varchar(30) DEFAULT NULL,`suvr_frontal_inf_tri_r` varchar(30) DEFAULT NULL,`suvr_frontal_inf_orb_l` varchar(30) DEFAULT NULL,`suvr_frontal_inf_orb_r` varchar(30) DEFAULT NULL,`suvr_rolandic_oper_l` varchar(30) DEFAULT NULL,`suvr_rolandic_oper_r` varchar(30) DEFAULT NULL,`suvr_supp_motor_area_l` varchar(30) DEFAULT NULL,`suvr_supp_motor_area_r` varchar(30) DEFAULT NULL,`suvr_olfactory_l` varchar(30) DEFAULT NULL,`suvr_olfactory_r` varchar(30) DEFAULT NULL,`suvr_frontal_sup_medial_l` varchar(30) DEFAULT NULL,`suvr_frontal_sup_medial_r` varchar(30) DEFAULT NULL,`suvr_frontal_med_orb_l` varchar(30) DEFAULT NULL,`suvr_frontal_med_orb_r` varchar(30) DEFAULT NULL,`suvr_rectus_l` varchar(30) DEFAULT NULL,`suvr_rectus_r` varchar(30) DEFAULT NULL,`suvr_insula_l` varchar(30) DEFAULT NULL,`suvr_insula_r` varchar(30) DEFAULT NULL,`suvr_cingulum_ant_l` varchar(30) DEFAULT NULL,`suvr_cingulum_ant_r` varchar(30) DEFAULT NULL,`suvr_cingulum_mid_l` varchar(30) DEFAULT NULL,`suvr_cingulum_mid_r` varchar(30) DEFAULT NULL,`suvr_cingulum_post_l` varchar(30) DEFAULT NULL,`suvr_cingulum_post_r` varchar(30) DEFAULT NULL,`suvr_hippocampus_l` varchar(30) DEFAULT NULL,`suvr_hippocampus_r` varchar(30) DEFAULT NULL,`suvr_parahippocampal_l` varchar(30) DEFAULT NULL,`suvr_parahippocampal_r` varchar(30) DEFAULT NULL,`suvr_amygdala_l` varchar(30) DEFAULT NULL,`suvr_amygdala_r` varchar(30) DEFAULT NULL,`suvr_calcarine_l` varchar(30) DEFAULT NULL,`suvr_calcarine_r` varchar(30) DEFAULT NULL,`suvr_cuneus_l` varchar(30) DEFAULT NULL,`suvr_cuneus_r` varchar(30) DEFAULT NULL,`suvr_lingual_l` varchar(30) DEFAULT NULL,`suvr_lingual_r` varchar(30) DEFAULT NULL,`suvr_occipital_sup_l` varchar(30) DEFAULT NULL,`suvr_occipital_sup_r` varchar(30) DEFAULT NULL,`suvr_occipital_mid_l` varchar(30) DEFAULT NULL,`suvr_occipital_mid_r` varchar(30) DEFAULT NULL,`suvr_occipital_inf_l` varchar(30) DEFAULT NULL,`suvr_occipital_inf_r` varchar(30) DEFAULT NULL,`suvr_fusiform_l` varchar(30) DEFAULT NULL,`suvr_fusiform_r` varchar(30) DEFAULT NULL,`suvr_postcentral_l` varchar(30) DEFAULT NULL,`suvr_postcentral_r` varchar(30) DEFAULT NULL,`suvr_parietal_sup_l` varchar(30) DEFAULT NULL,`suvr_parietal_sup_r` varchar(30) DEFAULT NULL,`suvr_parietal_inf_l` varchar(30) DEFAULT NULL,`suvr_parietal_inf_r` varchar(30) DEFAULT NULL,`suvr_supramarginal_l` varchar(30) DEFAULT NULL,`suvr_supramarginal_r` varchar(30) DEFAULT NULL,`suvr_angular_l` varchar(30) DEFAULT NULL,`suvr_angular_r` varchar(30) DEFAULT NULL,`suvr_precuneus_l` varchar(30) DEFAULT NULL,`suvr_precuneus_r` varchar(30) DEFAULT NULL,`suvr_paracentral_lobule_l` varchar(30) DEFAULT NULL,`suvr_paracentral_lobule_r` varchar(30) DEFAULT NULL,`suvr_caudate_l` varchar(30) DEFAULT NULL,`suvr_caudate_r` varchar(30) DEFAULT NULL,`suvr_putamen_l` varchar(30) DEFAULT NULL,`suvr_putamen_r` varchar(30) DEFAULT NULL,`suvr_pallidum_l` varchar(30) DEFAULT NULL,`suvr_pallidum_r` varchar(30) DEFAULT NULL,`suvr_thalamus_l` varchar(30) DEFAULT NULL,`suvr_thalamus_r` varchar(30) DEFAULT NULL,`suvr_heschl_l` varchar(30) DEFAULT NULL,`suvr_heschl_r` varchar(30) DEFAULT NULL,`suvr_temporal_sup_l` varchar(30) DEFAULT NULL,`suvr_temporal_sup_r` varchar(30) DEFAULT NULL,`suvr_temporal_pole_sup_l` varchar(30) DEFAULT NULL,`suvr_temporal_pole_sup_r` varchar(30) DEFAULT NULL,`suvr_temporal_mid_l` varchar(30) DEFAULT NULL,`suvr_temporal_mid_r` varchar(30) DEFAULT NULL,`suvr_temporal_pole_mid_l` varchar(30) DEFAULT NULL,`suvr_temporal_pole_mid_r` varchar(30) DEFAULT NULL,`suvr_temporal_inf_l` varchar(30) DEFAULT NULL,`suvr_temporal_inf_r` varchar(30) DEFAULT NULL,`suvr_cerebelum_crus1_l` varchar(30) DEFAULT NULL,`suvr_cerebelum_crus1_r` varchar(30) DEFAULT NULL,`suvr_cerebelum_crus2_l` varchar(30) DEFAULT NULL,`suvr_cerebelum_crus2_r` varchar(30) DEFAULT NULL,`suvr_cerebelum_3_l` varchar(30) DEFAULT NULL,`suvr_cerebelum_3_r` varchar(30) DEFAULT NULL,`suvr_cerebelum_4_5_l` varchar(30) DEFAULT NULL,`suvr_cerebelum_4_5_r` varchar(30) DEFAULT NULL,`suvr_cerebelum_6_l` varchar(30) DEFAULT NULL,`suvr_cerebelum_6_r` varchar(30) DEFAULT NULL,`suvr_cerebelum_7b_l` varchar(30) DEFAULT NULL,`suvr_cerebelum_7b_r` varchar(30) DEFAULT NULL,`suvr_cerebelum_8_l` varchar(30) DEFAULT NULL,`suvr_cerebelum_8_r` varchar(30) DEFAULT NULL,`suvr_cerebelum_9_l` varchar(30) DEFAULT NULL,`suvr_cerebelum_9_r` varchar(30) DEFAULT NULL,`suvr_cerebelum_10_l` varchar(30) DEFAULT NULL,`suvr_cerebelum_10_r` varchar(30) DEFAULT NULL,`suvr_vermis_1_2` varchar(30) DEFAULT NULL,`suvr_vermis_3` varchar(30) DEFAULT NULL,`suvr_vermis_4_5` varchar(30) DEFAULT NULL,`suvr_vermis_6` varchar(30) DEFAULT NULL,`suvr_vermis_7` varchar(30) DEFAULT NULL,`suvr_vermis_8` varchar(30) DEFAULT NULL,`suvr_vermis_9` varchar(30) DEFAULT NULL,`suvr_vermis_10` varchar(30) DEFAULT NULL,`volume_cc_precentral_l` varchar(10) DEFAULT NULL,`volume_cc_precentral_r` varchar(10) DEFAULT NULL,`volume_cc_frontal_sup_l` varchar(10) DEFAULT NULL,`volume_cc_frontal_sup_r` varchar(10) DEFAULT NULL,`volume_cc_frontal_sup_orb_l` varchar(10) DEFAULT NULL,`volume_cc_frontal_sup_orb_r` varchar(10) DEFAULT NULL,`volume_cc_frontal_mid_l` varchar(10) DEFAULT NULL,`volume_cc_frontal_mid_r` varchar(10) DEFAULT NULL,`volume_cc_frontal_mid_orb_l` varchar(10) DEFAULT NULL,`volume_cc_frontal_mid_orb_r` varchar(10) DEFAULT NULL,`volume_cc_frontal_inf_oper_l` varchar(10) DEFAULT NULL,`volume_cc_frontal_inf_oper_r` varchar(10) DEFAULT NULL,`volume_cc_frontal_inf_tri_l` varchar(10) DEFAULT NULL,`volume_cc_frontal_inf_tri_r` varchar(10) DEFAULT NULL,`volume_cc_frontal_inf_orb_l` varchar(10) DEFAULT NULL,`volume_cc_frontal_inf_orb_r` varchar(10) DEFAULT NULL,`volume_cc_rolandic_oper_l` varchar(10) DEFAULT NULL,`volume_cc_rolandic_oper_r` varchar(10) DEFAULT NULL,`volume_cc_supp_motor_area_l` varchar(10) DEFAULT NULL,`volume_cc_supp_motor_area_r` varchar(10) DEFAULT NULL,`volume_cc_olfactory_l` varchar(10) DEFAULT NULL,`volume_cc_olfactory_r` varchar(10) DEFAULT NULL,`volume_cc_frontal_sup_medial_l` varchar(10) DEFAULT NULL,`volume_cc_frontal_sup_medial_r` varchar(10) DEFAULT NULL,`volume_cc_frontal_med_orb_l` varchar(10) DEFAULT NULL,`volume_cc_frontal_med_orb_r` varchar(10) DEFAULT NULL,`volume_cc_rectus_l` varchar(10) DEFAULT NULL,`volume_cc_rectus_r` varchar(10) DEFAULT NULL,`volume_cc_insula_l` varchar(10) DEFAULT NULL,`volume_cc_insula_r` varchar(10) DEFAULT NULL,`volume_cc_cingulum_ant_l` varchar(10) DEFAULT NULL,`volume_cc_cingulum_ant_r` varchar(10) DEFAULT NULL,`volume_cc_cingulum_mid_l` varchar(10) DEFAULT NULL,`volume_cc_cingulum_mid_r` varchar(10) DEFAULT NULL,`volume_cc_cingulum_post_l` varchar(10) DEFAULT NULL,`volume_cc_cingulum_post_r` varchar(10) DEFAULT NULL,`volume_cc_hippocampus_l` varchar(10) DEFAULT NULL,`volume_cc_hippocampus_r` varchar(10) DEFAULT NULL,`volume_cc_parahippocampal_l` varchar(10) DEFAULT NULL,`volume_cc_parahippocampal_r` varchar(10) DEFAULT NULL,`volume_cc_amygdala_l` varchar(10) DEFAULT NULL,`volume_cc_amygdala_r` varchar(10) DEFAULT NULL,`volume_cc_calcarine_l` varchar(10) DEFAULT NULL,`volume_cc_calcarine_r` varchar(10) DEFAULT NULL,`volume_cc_cuneus_l` varchar(10) DEFAULT NULL,`volume_cc_cuneus_r` varchar(10) DEFAULT NULL,`volume_cc_lingual_l` varchar(10) DEFAULT NULL,`volume_cc_lingual_r` varchar(10) DEFAULT NULL,`volume_cc_occipital_sup_l` varchar(10) DEFAULT NULL,`volume_cc_occipital_sup_r` varchar(10) DEFAULT NULL,`volume_cc_occipital_mid_l` varchar(10) DEFAULT NULL,`volume_cc_occipital_mid_r` varchar(10) DEFAULT NULL,`volume_cc_occipital_inf_l` varchar(10) DEFAULT NULL,`volume_cc_occipital_inf_r` varchar(10) DEFAULT NULL,`volume_cc_fusiform_l` varchar(10) DEFAULT NULL,`volume_cc_fusiform_r` varchar(10) DEFAULT NULL,`volume_cc_postcentral_l` varchar(10) DEFAULT NULL,`volume_cc_postcentral_r` varchar(10) DEFAULT NULL,`volume_cc_parietal_sup_l` varchar(10) DEFAULT NULL,`volume_cc_parietal_sup_r` varchar(10) DEFAULT NULL,`volume_cc_parietal_inf_l` varchar(10) DEFAULT NULL,`volume_cc_parietal_inf_r` varchar(10) DEFAULT NULL,`volume_cc_supramarginal_l` varchar(10) DEFAULT NULL,`volume_cc_supramarginal_r` varchar(10) DEFAULT NULL,`volume_cc_angular_l` varchar(10) DEFAULT NULL,`volume_cc_angular_r` varchar(10) DEFAULT NULL,`volume_cc_precuneus_l` varchar(10) DEFAULT NULL,`volume_cc_precuneus_r` varchar(10) DEFAULT NULL,`volume_cc_paracentral_lobule_l` varchar(10) DEFAULT NULL,`volume_cc_paracentral_lobule_r` varchar(10) DEFAULT NULL,`volume_cc_caudate_l` varchar(10) DEFAULT NULL,`volume_cc_caudate_r` varchar(10) DEFAULT NULL,`volume_cc_putamen_l` varchar(10) DEFAULT NULL,`volume_cc_putamen_r` varchar(10) DEFAULT NULL,`volume_cc_pallidum_l` varchar(10) DEFAULT NULL,`volume_cc_pallidum_r` varchar(10) DEFAULT NULL,`volume_cc_thalamus_l` varchar(10) DEFAULT NULL,`volume_cc_thalamus_r` varchar(10) DEFAULT NULL,`volume_cc_heschl_l` varchar(10) DEFAULT NULL,`volume_cc_heschl_r` varchar(10) DEFAULT NULL,`volume_cc_temporal_sup_l` varchar(10) DEFAULT NULL,`volume_cc_temporal_sup_r` varchar(10) DEFAULT NULL,`volume_cc_temporal_pole_sup_l` varchar(10) DEFAULT NULL,`volume_cc_temporal_pole_sup_r` varchar(10) DEFAULT NULL,`volume_cc_temporal_mid_l` varchar(10) DEFAULT NULL,`volume_cc_temporal_mid_r` varchar(10) DEFAULT NULL,`volume_cc_temporal_pole_mid_l` varchar(10) DEFAULT NULL,`volume_cc_temporal_pole_mid_r` varchar(10) DEFAULT NULL,`volume_cc_temporal_inf_l` varchar(10) DEFAULT NULL,`volume_cc_temporal_inf_r` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_crus1_l` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_crus1_r` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_crus2_l` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_crus2_r` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_3_l` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_3_r` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_4_5_l` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_4_5_r` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_6_l` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_6_r` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_7b_l` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_7b_r` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_8_l` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_8_r` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_9_l` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_9_r` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_10_l` varchar(10) DEFAULT NULL,`volume_cc_cerebelum_10_r` varchar(10) DEFAULT NULL,`volume_cc_vermis_1_2` varchar(10) DEFAULT NULL,`volume_cc_vermis_3` varchar(10) DEFAULT NULL,`volume_cc_vermis_4_5` varchar(10) DEFAULT NULL,`volume_cc_vermis_6` varchar(10) DEFAULT NULL,`volume_cc_vermis_7` varchar(10) DEFAULT NULL,`volume_cc_vermis_8` varchar(10) DEFAULT NULL,`volume_cc_vermis_9` varchar(10) DEFAULT NULL,`volume_cc_vermis_10` varchar(10) DEFAULT NULL,`age_at_appointment` varchar(10) DEFAULT NULL,`bias_corrected_t1_mri_file` varchar(200) DEFAULT NULL,`mni_space_t1_mri` varchar(200) DEFAULT NULL,`multispectral_file` varchar(200) DEFAULT NULL,`pet_date_mri_date_diff_days` varchar(30) DEFAULT NULL,`mri_processing_date` varchar(30) DEFAULT NULL,`pet_log_file` varchar(200) DEFAULT NULL,`coregistration_reference_file` varchar(200) DEFAULT NULL,`processed_4d_pet_file` varchar(200) DEFAULT NULL,`processed_sum_pet_file` varchar(200) DEFAULT NULL,`processed_suvr_pet_file` varchar(200) DEFAULT NULL,`mni_space_suvr_pet_file` varchar(200) DEFAULT NULL,`pet_analysis_log_file` varchar(200) DEFAULT NULL)
