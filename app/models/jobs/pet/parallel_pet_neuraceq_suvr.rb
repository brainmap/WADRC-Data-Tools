class Jobs::Pet::ParallelPetNeuraceqSuvr < Jobs::Pet::PetBase
  ...
  def self.default_params
    params = { schedule_name: 'parallel_pet_neuraceq_suvr_process',
                ...
                uptake_range: [85,95],
                sp_pet_selection: "include",
                sp_mri_selection: "exclude",
                exclude_sp_mri_array: [-1,100,80,76,78],
                exclude_sp_pet_array: [-1,100],
                include_sp_mri_array: [155], #only asthana.wls.visit1
                include_sp_pet_array: [155]
              }
    params.default = ''
    params
  end
  ...
  def self.production_params
  	params = { schedule_name: 'parallel_pet_neuraceq_suvr_process',
                ...
                pipeline_id: 200,
                run_no_job_case: true, #once pipelines table is set up then set to false
                logger_dir: '/mounts/data/preprocessed/logs/pet_neuraceq',
                days_mri_pet_diff_limit: "730",
                uptake_range: [85,95],
                sp_pet_selection: "include",
  ...
  def run(params)
    ...
    begin
      @driver_logger = ActiveSupport::Logger.new("#{params[:logger_dir]}/#{Time.now.strftime("%Y-%m-%d_%H:%M:%S")}_neuraceq_driver.log")
...
###TODO### Create write driver method seperate from matlab_call method
#      write_driver(params)
...
#      @driver_logger.close()
      ...
      @driver_logger.error{"Error (#{error.class}): #{error.message}"}
#      @driver_logger.close()
  ...
  def setup(params)
      ...
      @driver_logger.info{"Pet Tracer Found: #{@pettracer.name}; LookupPettracer.id: #{@pettracer.id}"}

      @preprocessed_tracer_path = "/pet/#{@pettracer.name.downcase}/#{params[:method]}/code_ver2b"
      @driver_logger.info{"Preprocessed Tracer Path: #{@preprocessed_tracer_path}"}
  ...
  def selection(params)
    ...
    if params[:sp_pet_selection] == "exclude"

      @petscans = Jobs::Pet::PetscanNeuraceq.where("petscans.lookup_pettracer_id in (?) 
      ...
      @petscans.each do |pet_appt|
        self.inputs << {:class => pet_appt.class, :id => pet_appt.id} #"{\"class\":\"#{pet_appt.class}\",\"id\":\"#{pet_appt.id}\"}"
      end

    elsif params[:sp_pet_selection] == "include"
      ...
      @petscans = Jobs::Pet::PetscanNeuraceq.where("petscans.lookup_pettracer_id in (?) 
                  ...
                  where appointments.vgroup_id = scan_procedures_vgroups.vgroup_id
                  and scan_procedures_vgroups.scan_procedure_id in (?))",params[:tracer_id],params[:include_sp_pet_array])
    ...
    petscans_ids = []
    @petscans.each{ |scan| petscans_ids.push(scan.id) }
    @driver_logger.info{"#{@petscans.size} Pet Scans Found;\n #{petscans_ids.to_s} "}
  ...
  def filter(params)
    ...
    @petscans.each do |pet_appt|
      ...
      #Prelim check so pt_log and job_case can be made
      if pet_appt.related_enumber.nil? or pet_appt.related_scan_procedure.nil?
        @driver_logger.info{"EXCLUSION: #{pet_appt.id}; appointment had broken relations to enrollment or scan procedures"}
#        self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "appointment had broken relations to enrollment or scan procedures"}
        next
      end

      pt_log_path = "#{params[:logger_dir]}/#{pet_appt.related_scan_procedure.codename.gsub('.','_')}_#{pet_appt.related_enumber.enumber}_#{Time.now.strftime("%Y-%m-%d_%H:%M:%S")}_neuraceq_driver.log"

      pt_log = ActiveSupport::Logger.new(pt_log_path)
      pt_log.info{"FILTER: Considering enumber: #{pet_appt.related_enumber.enumber}; Pet Scan ID: #{pet_appt.id}"}
          ...
          if !v_pet_appointment.vgroup_id.nil?
            ...
            if !vgroup.participant_id.nil?
              ...
              pt_log.error{"Vgroup had a broken participant_id; Vgroup_id: #{v_pet_appointment.vgroup_id}; PetScan_id: #{pet_appt.id}"}
              @driver_logger.info{"EXCLUSION: #{pet_appt.id}; Vgroup had a broken participant_id"}
#              pt_log.close()
#              self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "vgroup had a broken participant_id"}
              next
            end
          else
            pt_log.error{"Appointment had a broken Vgroup_id; Vgroup_id: #{v_pet_appointment.vgroup_id}; PetScan_id: #{pet_appt.id}"}
#            pt_log.close()
            @driver_logger.info{"EXCLUSION: #{pet_appt.id}; Appointment had a broken Vgroup_id"}
#            self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "appointment had a broken vgroup_id"}
            next
          end
      ...
      if !params[:run_no_job_case]
        ...
        #use new find_or_create_by method from JobCase
        @job_case = Jobs::JobCase.find_or_create_by(:pipeline_id => pipeline.id, 
                                                    :enrollment_id => pet_appt.related_enumber.id, 
                                                    :scan_procedure_id => pet_appt.related_scan_procedure.id)

        #        if job_case.empty?
        #          job_case = Jobs::JobCase.new(:pipeline_id => pipeline.id, :enrollment_id => enrollment.id, 
        #                                       :enumber => enrollment.enumber, :scan_procedure_id => scan_procedure.id, 
        #                                       :status =>'started', :job_run_id => self.job_run.id)
        #          
        #          if !job_case.save()
        #            puts "uh oh!"
        #            @driver_logger.error{"JobCase could not be saved for Vgroup. Needs to be resolved! Skipping! class: #{vgroup.class}; id: #{vgroup.id}; enumber: #{enrollment.enumber}; procedure: #{scan_procedure.codename}"}
        #            next
        #          end

        if job_case.count == 1
          if job_case.first.status == 'exclusion'
            @driver_logger.info{"EXCLUSION: Vgroup has JobCase status of 'exclusion'. Skipping! class: #{vgroup.class}; id: #{vgroup.id}; enumber: #{pet_appt.related_enumber.enumber}; procedure: #{pet_appt.related_scan_procedure.codename}"}
            pt_log.error{"EXCLUSION: Vgroup has JobCase status of 'exclusion'. Skipping! class: #{vgroup.class}; id: #{vgroup.id}; enumber: #{pet_appt.related_enumber.enumber}; procedure: #{pet_appt.related_scan_procedure.codename}"}
            next
            #          else
            #            @driver_logger.error{"Vgroup has an open JobCase (id: #{job_case.first.id}) with a status of #{job_case.first.status}. Needs to be resolved! Skipping! class: #{vgroup.class}; id: #{vgroup.id}; enumber: #{enrollment.enumber}; procedure: #{scan_procedure.codename}"}
          end
          #          next
        elsif job_case.count > 1
          ids = job_case.map{ |job| job.id }
          @driver_logger.error{"Vgroup has multiple JobCases (ids: #{ids.to_s}). Needs to be resolved! Skipping! class: #{vgroup.class}; id: #{vgroup.id}; enumber: #{pet_appt.related_enumber.enumber}; procedure: #{pet_appt.related_scan_procedure.codename}"}
          pt_log.error{"Vgroup has multiple JobCases (ids: #{ids.to_s}). Needs to be resolved! Skipping! class: #{vgroup.class}; id: #{vgroup.id}; enumber: #{pet_appt.related_enumber.enumber}; procedure: #{pet_appt.related_scan_procedure.codename}"}
#          self.exclusions << {:vgroup_id => vgroup.id, :petscans_id => pet_appt.id, :message => "Vgroup has multiple JobCases (ids: #{ids.to_s}). Needs to be resolved! Skipping!"}
          next
        end
      end
...
#            end
            ...
            pt_log.info{"Added to okay_paths_and_not_processed_before"}
            ...
            if pet_appt.o_acpc_file_exists?
              ...
              pt_log.info{"ACPC file does not exist in #{pet_appt.related_enumber.enumber}'s Preprocessed/unknown directory"}
              #then look around in nearby visits for this participant for an o_acpc.nii file
              if params[:sp_mri_selection] == "exclude"
              ...
              elsif params[:sp_mri_selection] == "include"
                recent_mri_visits = Visit.where("visits.appointment_id in (select appointments.id from appointments join vgroups on appointments.vgroup_id = vgroups.id 
                  where vgroups.participant_id in (?)
                  and vgroups.transfer_mri = 'yes' and appointments.appointment_type = 'mri'
                  and abs(datediff(appointments.appointment_date,?))  < "+params[:days_mri_pet_diff_limit]+"
                  and vgroups.id in 
                  (select scan_procedures_vgroups.vgroup_id from scan_procedures_vgroups 
                     where scan_procedures_vgroups.scan_procedure_id in (?)) )", pet_appt.related_participant.id,pet_appt.related_appointment.appointment_date.to_s,params[:include_sp_mri_array]).order("date desc")
              end
              ...
              if pet_appt.recent_o_acpc_file_exists?(recent_mri_visits)
                ...
                pt_log.info{"Could not find recent ACPC file either. EXCLUDED"}
#                pt_log.close()
                @driver_logger.info{"EXCLUSION: #{pet_appt.id}; no recent o_acpc files"}
#                self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "no recent o_acpc files"}
            ...
            if pet_appt.multispectral_file_exists?
              ...
              if pet_appt.recent_multispectral_file_exists?(recent_mri_visits)
                ...
                pt_log.info{"Could not find recent multispectral file either. EXCLUDED"}
#                pt_log.close()
                @driver_logger.info{"EXCLUSION: #{pet_appt.id}; no recent multispectral files"}
#                self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "no multispectral file"}
            ...
            if params[:method] == 'suvr'
              ...
              if !o_acpc_file_path.blank? && !multispectral_file_path.blank? && !pet_appt.scanstarttime.nil? && !pet_appt.injecttiontime.nil?
                ...
                if (v_uptake_duration.to_i).between?(params[:uptake_range][0],params[:uptake_range][1])
                  ...
                  pt_log.info{":proto => #{proto}, :visit => #{visit}, :enum => #{pet_appt.related_enumber.enumber}, :t_uptake => #{v_uptake_duration}, :file_t1 => #{o_acpc_file_path}, :file_mult => #{multispectral_file_path}, :norm_samp => 1.0"}
#                  self.outputs << {:proto => proto, :visit => visit, :enum => pet_appt.related_enumber.enumber, :t_uptake => v_uptake_duration, :file_t1 => o_acpc_file_path, :file_mult => multispectral_file_path, :norm_samp => 1.0}
                else
                  pt_log.info{"Bad uptake duration (#{v_uptake_duration.to_i}). EXCLUDED"}
#                  pt_log.close()
                  @driver_logger.info{"EXCLUSION: #{pet_appt.id}; Bad uptake duration"}
#                  self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "bad uptake duration (#{v_uptake_duration.to_i})"}
                end
              end
              ...
              if !o_acpc_file_path.blank? && !multispectral_file_path.blank?
                ...
                pt_log.info{":proto => #{proto}, :visit => #{visit}, :enum => #{pet_appt.related_enumber.enumber}, :file_t1 => #{o_acpc_file_path}, :file_mult => #{multispectral_file_path}, :norm_samp => 1.0"}
#                self.outputs << {:proto => proto, :visit => visit, :enum => pet_appt.related_enumber.enumber, :file_t1 => o_acpc_file_path, :file_mult => multispectral_file_path, :norm_samp => 1.0}
...
#            pt_log.close()
            ...
            if !pet_appt.paths_ok?
              pt_log.info{"Paths not ok. EXCLUDED"}
              @driver_logger.info{"EXCLUSION: #{pet_appt.id}; Paths not ok."}
#              self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "paths not ok"}
            elsif pet_appt.preprocessed_dir_exists?(@preprocessed_tracer_path)
              pt_log.info{"Already processed. EXCLUDED"}
              @driver_logger.info{"EXCLUSION: #{pet_appt.id}; Already processed."}
#              self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "already processed"}
            end
            ...
            if pet_appt.related_scan_procedure.nil? or pet_appt.related_enumber.nil?
              ...
              pt_log.info{"Appointment had broken relations to enrollment or scan procedures . EXCLUDED"}
#              pt_log.close()
              next
            end
            begin
              ...
              pt_log.info{"No ecat files. EXCLUDED"}
              @driver_logger.info{"EXCLUSION: #{pet_appt.id}; No ecat files."}
#              pt_log.close()
#              self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "no ecat files"}

            rescue Exceptions::PetscanTooManyEcatsError => e

              pt_log.info{"Too many ecat files. EXCLUDED"}
              @driver_logger.info{"EXCLUSION: #{pet_appt.id}; Too many ecat files."}
#              pt_log.close()
#              self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "too many ecat files"}

            rescue Exceptions::PetscanError, Exceptions::PetscanPathError => e

              pt_log.info{"Just weird: #{e.message}. EXCLUDED"}
              @driver_logger.info{"EXCLUSION: #{pet_appt.id}; Just weird: #{e.message}."}
#              pt_log.close()
#              self.exclusions << {:vgroup_id => v_pet_appointment.vgroup_id, :petscans_id => pet_appt.id, :message => "Just weird: #{e.message}"}
      ...
      if !params[:dry_run]
        ...
        driver.each do |row|
          ...
          if !File.directory?(visit_dir)
            Dir.mkdir(visit_dir)
            @driver_logger.info{"PROCESS: Made #{visit_dir}."}
          end
# If matlab job returns a permissions error try running fix rights on #{params[:base_path]}/preprocessed/visits/#{row[:proto]}.#{row[:visit]}/#{row[:enum]}
# MATLAB job may be having trouble creating the below directory
#          neura_output = "#{params[:base_path]}/preprocessed/visits/#{row[:proto]}.#{row[:visit]}/#{row[:enum]}/pet/#{@pettracer.name.downcase}/suvr"
#          if !File.directory?(neura_output)
#            `mkdir -p #{neura_output}`
#          end
        end
        ...
        @driver_logger.info{"PROCESS: Process Call: #{v_call}."}
#        self.log << {:message => v_call}
#        self.job_run.save

        begin
          require 'open3'
          stdin, stdout, stderr, wait_thr = Open3.popen3(v_call)
          while !stdout.eof?
            v_output = stdout.read 1024 
            #  v_comment = v_comment + v_output  
            @driver_logger.info{"PROCESS: #{v_output}"}
#            puts v_output
#            self.log << v_output
          end
          ...
          @driver_logger.info{"PROCESS: PID: #{pid}; Errors: #{error}; Exit_Value: #{exit_value}"}
          ...
          @driver_logger.error{"PROCESS: #{v_comment}"}
#          self.log << v_comment
        end
#        @driver_logger.close()
        # v_success ="N"
