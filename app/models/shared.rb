 ...
 #                                                                                               end } 
  ...
  def run_adrc_upload  
    ...
    results.each do |r|
          ...
          sql2 = "insert into cg_adrc_upload_new (subjectid,sent_flag,status_flag, enrollment_id, scan_procedure_id,dti_sent_flag,dti_status_flag,pcvipr_sent_flag,pcvipr_status_flag) values('"+r[0]+"','N','Y', "+enrollment[0].id.to_s+",22,'N','Y','N','Y')"
                         ...
                         if !enrollment.blank?
                             ...
                             # multiple asl dircectory from image_datasets.path 
                              sql_dir = "select distinct SUBSTRING_INDEX(image_datasets.path,'/',-1) from image_datasets,visits v, appointments a, scan_procedures_vgroups spv, enrollment_vgroup_memberships evm
                                   ...
                                   and evm.enrollment_id ="+enrollment[0].id.to_s+" and a.vgroup_id = evm.vgroup_id and image_datasets.series_description not like '%Cerebral Blood Flow%' and image_datasets.series_description not like '%ASL CBF%'
...
# added adrc native below  --- none of the adrc are in the regular  -- but check 
  ...
  def run_goveas_20131031_upload
    ...
    sql = "select distinct subjectid, enrollment_id, scan_procedure_id, export_id from cg_goveas_20131031
           where ( done_flag != 'Y' or done_flag is NULL)
           and ( status_flag = 'R' or status_flag is NULL) 
    ...
    results.each do |r|
      ...
      results_dataset.each do |r_dataset|
            ...
            v_path = r_dataset[4]
            v_dir_array = v_path.split("/")
            v_actual_scan_procedure = v_dir_array[3]
            if v_dir_array[4] == "mri"
